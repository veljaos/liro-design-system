...
| D2  | No translation and no text of its own: components show the text they are given; their own few strings come from provider `messages` (English defaults in `packages/ui/src/provider/messages.en.ts`).            | Review; the `LiroMessages` type makes a missing key a compile error (proved in `packages/ui/src/provider/provider-p15.test.tsx`)                                  |
| D3  | No permission decisions: show what you are told — hidden, read-only, or disabled with a visible reason.                                                                                                         | Review                                                                                                                                                            |
| D4  | Money and numbers are decimal strings. Never convert to a JavaScript `number`, never add, round or truncate; totals arrive computed. Unreadable input parses to `null`, never `0`.                              | Unit tests in `packages/ui/src/provider/format.test.ts` and `parse-and-dates.test.ts` (no rounding, no loss, every scheme read back, unreadable → `null`); review |
| D5  | No business domain inside components ("invoice", "VAT", "PIB" …). Fictitious examples in stories are fine.                                                                                                      | Review                                                                                                                                                            |
| D6  | No raw colours: only semantic names (`bg-destructive`, `text-secondary`). No `bg-red-500`, no `text-[#…]`, no hex outside `packages/tokens`.                                                                    | Review until P1.1 adds the colour rule to `@veljaos/eslint-config`, with fixture tests                                                                            |
| D7  | Three token layers: values → meanings → components. Components choose a meaning, never a value. Buttons choose an **intent** or a **family**, never a colour or variant.                                        | Review; from P1.3 the `Button` props have no `color` or `variant`, so the types enforce it                                                                        |
| D8  | Layout (width, alignment, spacing to neighbours) may be set freely with Tailwind; colour may not (D6).                                                                                                          | Review                                                                                                                                                            |
| D9  | Logical properties only: `ms-`/`me-`/`ps-`/`pe-`/`start-`/`end-`/`text-start`; never `ml-`/`left-`/`text-left`. Pointer and keyboard movement measures from the leading edge.                                   | Review; the ltr/rtl runs of the visual and accessibility tests. No lint rule yet: plan section 6 requires one, to be added with the P1.1 colour rule              |
...
| D11 | Everything that differs between applications, languages and customers goes through `LiroProvider` (locale, direction, messages, format, today, week start, colour scheme, link component), each with a default. | Review; provider tests in `packages/ui/src/provider/liro-provider.test.tsx` and `provider-p15.test.tsx`                                                           |
...
| Name                                                                                                                                                                                                                   | Kind      | Step | Note                                                                                 |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---- | ------------------------------------------------------------------------------------ |
| `LiroProvider`, `useLiro`                                                                                                                                                                                              | Provider  | P0.3 | Complete (P1.5): every prop of plan section 5; sets Radix direction.                 |
| `createFormat`, `formatDecimal`, `parseDecimal`, `parseDateText`, `dateFieldOrder`, `intlLocale`, `directionForLocale`, `numberSchemeForLocale`, `weekStartsOnForLocale`, `localToday`, `NUMBER_SCHEMES`, `messagesEn` | Helpers   | P0.3 | Formatting and parsing on decimal strings; dates; the English messages.              |
